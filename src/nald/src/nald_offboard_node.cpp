...
{
    ros::init(argc, argv, "nald_offboard_node");
    ros::NodeHandle nh;

    ros::Subscriber     state_sub        = nh.subscribe<mavros_msgs::State>("mavros/state", 10, cbState);

    ros::Subscriber     move_x_sub        = nh.subscribe("nald/x_pos", 10, cbMoveX);
    ros::Subscriber     move_y_sub        = nh.subscribe("nald/y_pos", 10, cbMoveY);
    ros::Subscriber     move_z_sub        = nh.subscribe("nald/z_pos", 10, cbMoveZ);
