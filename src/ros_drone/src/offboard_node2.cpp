...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber gpsX_sub = nodeHandle.subscribe<std_msgs::Float64>("gps_X", 10, cbMoveX);
    ros::Subscriber gpsY_sub = nodeHandle.subscribe<std_msgs::Float64>("gps_Y", 10, cbMoveY);
    ros::Subscriber gpsZ_sub = nodeHandle.subscribe<std_msgs::Float64>("gps_Z", 10, cbMoveZ);
...
void cbMoveX(const std_msgs::Float64::ConstPtr& msg)
{
  ...
  double pos = msg->data;
  // double pos = std::stod(msg->data);
  std::cout << "cbMoveX: " << pos << std::endl;
...
void cbMoveX(const std_msgs::Float64::ConstPtr& msg)
{
  ...
  g_pose.pose.position.x = msg->data - original_latitude;
...
void cbMoveY(const std_msgs::Float64::ConstPtr& msg)
{
  ...
  g_pose.pose.position.y = msg->data - original_longitude;
